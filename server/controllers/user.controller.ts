...
export const supabaseLogin = async (req: Request, res: Response) => {
  console.log("[SupabaseLogin] Request received at:", new Date().toISOString());
  console.log("[SupabaseLogin] Request body:", { 
    email: req.body?.email, 
    hasName: !!req.body?.name,
    hasAvatar: !!req.body?.avatar,
    hasSupabaseUserId: !!req.body?.supabaseUserId 
  });
  ...
  try {
    ...
    if (!email || !supabaseUserId) {
      console.log("[SupabaseLogin] Missing required fields");
      return res.status(400).json({
        success: false,
    ...
    console.log("[SupabaseLogin] Checking for existing user...");
    ...
    console.log("[SupabaseLogin] User lookup result:", user ? "Found existing user" : "User not found");

    if (!user) {
      console.log("[SupabaseLogin] Creating new user...");
      // Create new user
      user = await prisma.user.create({
        data: {
          email,
      ...
      console.log("[SupabaseLogin] New user created:", user.id);
    } else {
      console.log("[SupabaseLogin] Updating existing user...");
      ...
      console.log("[SupabaseLogin] User updated:", user.id);
    }

    console.log("[SupabaseLogin] Sending token...");
    await sendToken(user, res);
    console.log("[SupabaseLogin] Login successful!");
  } catch (error: any) {
