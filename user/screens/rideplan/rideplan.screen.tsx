...
export default function RidePlanScreen() {
  ...
  // Handle map tap - set marker at tapped location
  const handleMapPress = async (event: any) => {
    ...
    // Update map region to center on tapped location
    setRegion({
      ...region,
      latitude,
      longitude,
    ...
    // Reverse geocode to get address and update query
    ...
    try {
      ...
      console.error("[handleMapPress] Error getting address:", error);
    } finally {
      setIsUpdatingAddress(false);
    }

    // Fetch travel times if current location is available
    if (currentLocation) {
      fetchTravelTimes(currentLocation, newMarker).catch((error) => {
        console.error("[handleMapPress] Error fetching travel times:", error);
      });
    }
  };
  ...
  const handleMarkerDragEnd = async (event: any) => {
    ...
    try {
      ...
      console.error("[handleMarkerDragEnd] Error getting address:", error);
    } finally {
      setIsUpdatingAddress(false);
    }
    ...
    if (currentLocation) {
      fetchTravelTimes(currentLocation, newMarker).catch((error) => {
        console.error("[handleMarkerDragEnd] Error fetching travel times:", error);
      });
    }
  };
  ...
  const requestNearbyDrivers = async () => {
    ...
    if (!marker) {
      ...
      setdriverLoader(false);
      Toast.show("Please select a destination first.", {
        type: "info",
        placement: "bottom",
      });
      return;
    ...
    if (!currentLocation) {
      ...
      setdriverLoader(false);
      Toast.show("Please wait for your location to be detected.", {
        type: "info",
        placement: "bottom",
      });
      return;
    ...
    // Check WebSocket connection - try to reconnect if needed
    if (!ws.current) {
    ...
    const isWsOpen = ws.current && ws.current.readyState === WebSocket.OPEN;
    ...
    if (!isWsOpen) {
      ...
      if (isStillClosed) {
        ...
        setdriverLoader(false);
        Toast.show("Unable to connect to server. Please check your internet connection and try again.", {
          type: "danger",
          placement: "bottom",
        });
        return;
    ...
    try {
      // Get location names for the request
      ...
      try {
        ...
        currentLocationName = currentLocationRes.data.results[0]?.formatted_address || "";
      } catch (e) {
        console.log("[requestNearbyDrivers] Failed to get current location name:", e);
      }
      ...
      try {
        ...
        destinationLocationName = destinationRes.data.results[0]?.formatted_address || "";
      } catch (e) {
      ...
      const errorMessage = error?.message?.includes("connection") || error?.message?.includes("WebSocket")
        ? "Connection error. Please check your internet and try again."
        : "Failed to request drivers. Please try again.";
            ...
            <>
              ...
              >
                ...
                >
                  ...
                  >
                    <TextInput
                      placeholder="Where to? (Tap map or drag marker to select)"
                      value={query}
